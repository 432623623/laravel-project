...
<div x-data="{isOpen: false}">
    <span x-on:click="isOpen = true; document.querySelector('.chat-field').focus()" 
      ...
      title="Chat" data-toggle="tooltip" 
      data-placement="bottom">
      <i class="fas fa-comment"></i>
    </span>
    <div id="chat-wrapper" data-username="{{auth()->user()->username}}" 
    ...
    <div id="chat" class="chat-log">
        ...
        <a href="/profile/{{$chat['username']}}">
          <img class="avatar-tiny" src="{{$chat['avatar']}}"></a>
    ...
    <form wire:submit="send" id="chatForm" class="chat-form border-top">
      <input wire:model="textvalue" type="text" class="chat-field" 
      id="chatField" placeholder="Type a message…" autocomplete="off">
    </form>
    </div>
</div>
    ...
    <script>
      document.addEventListener('livewire:initialized', () => {
          Livewire.hook('morph.added', (element) => {
              if (element.el.classList.contains("chat-self") 
              || element.el.classList.contains("chat-other")) {
